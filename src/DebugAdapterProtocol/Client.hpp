...
public:
    ...
    /**
     * @brief The �disconnect� request is sent from the client to the debug adapter to terminate the debug session.
     * @param terminate true to terminate the debuggee, false to detach
     * @param restart true to restart the debuggee
     */
    void Disconnect(bool terminate = false, bool restart = false);  // (ph 25/10/26)

    /**
     * @brief The 'terminate' request is sent from the client to the debug adapter in order to shut down the debuggee gracefully.
     * @param restart true if the terminate request is part of a restart sequence
     */
     ...
     */
    void StepIn(int threadId = wxNOT_FOUND, bool singleThread = true, SteppingGranularity granularity = SteppingGranularity::LINE);
     ...
     * (e.g. the caller wants to associate the evaluated expression with the expression)
     *
    ...
    //(ph 2024/08/19)
    void GetDisassemble(uint64_t memoryAddr, int offset, int instructionOffset,
                                 int instructionCount, bool resolveSymbols);
    void GetVariableInfo(int variablesReference, wxString filter = "", int start = 0, int count = 1,
                            ValueDisplayFormat format = ValueDisplayFormat::NATIVE); //(ph 2024/10/10)
    void Goto(int threadId, int targetId); //(ph 2024/11/08)
    void GotoTarget(wxString source, int line, int column); //(ph 2024/11/08)
