...
struct WXDLLIMPEXP_DAP AttachRequestArguments : public Any {
    int pid = pid;
    // Remove the 'arguments' vector as it's not part of the standard DAP specification
...
struct WXDLLIMPEXP_DAP TerminateArguments : public Any {
    bool restart = false;
    ANY_CLASS(TerminateArguments);
    JSON_SERIALIZE();
};

struct WXDLLIMPEXP_DAP TerminateRequest : public Request {
    TerminateArguments arguments;
    REQUEST_CLASS(TerminateRequest, "terminate");
    JSON_SERIALIZE();
};

struct WXDLLIMPEXP_DAP TerminateResponse : public EmptyAckResponse {
    RESPONSE_CLASS(TerminateResponse, "terminate");
};
...
struct WXDLLIMPEXP_DAP StackFrame : public Any {
    ...
    // The instructionPointerReference //(ph 2024/08/28)
    wxString instructionPointerReference;
...
struct WXDLLIMPEXP_DAP VariablesArguments : public Any {
    ...
    int variablesReference = 0;
    ValueFormat format;
    //(ph 2024/10/10) start mod
    int start = 0;
    int count = 0;
    std::string filter = "";
    //(ph 2024/10/10) end mod
...
struct WXDLLIMPEXP_DAP DisassembleArguments : public Any {
    std::string memoryReference = "";
    int offset = 0;
    int instructionOffset = 0;
    int instructionCount = 0;
    bool resolveSymbols = true;
    ANY_CLASS(DisassembleArguments);
    JSON_SERIALIZE();
};

/// The request returns a disassemble response.
struct WXDLLIMPEXP_DAP DisassembleRequest : public Request {
    DisassembleArguments arguments;
    REQUEST_CLASS(DisassembleRequest, "disassemble");
    JSON_SERIALIZE();
};
/// Disassemble info returned in a response //(ph 2024/09/02) //(ph 2024/09/02)
struct WXDLLIMPEXP_DAP DisassembleInfo : public Any {

    wxString address;
    int column = -1; // optional
    wxString instruction;
    wxString instructionBytes;
    int line;
    //location object
    wxString name;
    wxString path;

    bool operator==(const DisassembleInfo& other) const
    {
        return address == other.address && column == other.column
         && instruction == other.instruction && instructionBytes == other.instructionBytes
         && name == other.name && path == other.path;
        //-return name == other.name && path == other.path && sourceReference == other.sourceReference;
    }

    ANY_CLASS(DisassembleInfo);
    JSON_SERIALIZE();
};

/// The response of a disassemble request.
struct WXDLLIMPEXP_DAP DisassembleResponse : public Response {
    /**
     * The result of the disassemble request.
     */
     wxString result;
    /**
     * The optional type of the evaluate result.
    ...
    std::vector<DisassembleInfo> disassembleInfo;
    RESPONSE_CLASS(DisassembleResponse, "disassemble");
    JSON_SERIALIZE();
};
// ----------------------------------------------------------------------------
//  Goto
// ----------------------------------------------------------------------------
//(ph 2024/11/08) mod Goto start

/// Arguments for 'Goto' request.
struct WXDLLIMPEXP_DAP GotoArguments : public Any {
    int threadId = -1;
    int targetId = -1;
    ANY_CLASS(GotoArguments);
    JSON_SERIALIZE();
};

/// The request for Goto
struct WXDLLIMPEXP_DAP GotoRequest : public Request {
    GotoArguments arguments;
    REQUEST_CLASS(GotoRequest, "goto");
    JSON_SERIALIZE();
};

/// The response of a Goto request.
struct WXDLLIMPEXP_DAP GotoResponse : public Response {
    /**
     * Response to goto request.
     * This is just an acknowledgement, so no body field is required.
     */
    RESPONSE_CLASS(GotoResponse, "goto");
    JSON_SERIALIZE();
}; //(ph 2024/11/08) mod 'Goto' end
// ----------------------------------------------------------------------------
//  GotoTargets
// ----------------------------------------------------------------------------
//(ph 2024/11/11) mod GotoTargets start

/// Arguments for 'GotoTargets' request.
struct WXDLLIMPEXP_DAP GotoTargetsArguments : public Any {
    Source source;
    int line = -1;
    int column = 1;
    int threadId = 0;
    ANY_CLASS(GotoTargetsArguments);
    JSON_SERIALIZE();
};

/// The request for GotoTargets
struct WXDLLIMPEXP_DAP GotoTargetsRequest : public Request {
    GotoTargetsArguments arguments;
    REQUEST_CLASS(GotoTargetsRequest, "gotoTargets");
    JSON_SERIALIZE();
};
/// GotoTargets info returned in a response //(ph 2024/11/11)
struct WXDLLIMPEXP_DAP GotoTargetsInfo : public Any {
    ...
    int      endLine   = -1;
    int      endColumn = -1;
    wxString instructionPointerReference = wxString();

    bool operator==(const GotoTargetsInfo& other) const
    {
        return id == other.id && line == other.line
         && column == other.column && endLine == other.endLine
         && instructionPointerReference == other.instructionPointerReference;
    }

    ANY_CLASS(GotoTargetsInfo);
    JSON_SERIALIZE();
};

/// The response of a GotoTargets request.
struct WXDLLIMPEXP_DAP GotoTargetsResponse : public Response
{
    /**
     * The result of the gotoTargs request.
     */
     wxString result;
    /**
     * The optional type of the evaluate result.
    ...
    std::vector<GotoTargetsInfo> gotoTargetsInfo;
    RESPONSE_CLASS(GotoTargetsResponse, "gotoTargets");
    JSON_SERIALIZE();
}; //(ph 2024/11/08) mod 'GotoTargets' end
// ----------------------------------------------------------------------------
};///end namespace dap
