...
// ----------------------------------------------------------------------------
{
    ...
    if (m_handshake_state != eHandshakeState::kCompleted)
    {
        ...
        if (msg && msg->type == "response" && msg->As<Response>()->command == "initialize")
        {
            ...
            ENABLE_FEATURE(supportsGotoTargetsRequest); //(ph 2024/11/12)
    ...
    else if (as_response)
    {
        if (as_response->command == "stackTrace")
        {
            ...
            if (not m_get_frames_queue.empty())
            {
        ...
        else if (as_response->command == "scopes")
        {
            ...
            if (not m_get_scopes_queue.empty())
            {
        ...
        else if (as_response->command == "variables")
        {
            ...
            if (not m_get_variables_queue.empty())
            {
        ...
        else if (as_response->command == "disassemble")
        {   //(ph 2024/08/26)
        ...
        else if (as_response->command == "gotoTargets")
        {   //(ph 2024/12/10)
            SendDAPEvent(wxEVT_DAP_GOTOTARGETS_RESPONSE, new dap::GotoTargetsResponse, json);
        }
        else if (as_response->command  == "attach")   // (ph 25/10/01)
        {
            wxString jsonDump = json.ToString(false);
            AttachResponse* ptr = new dap::AttachResponse;
            ptr->jsonDump = jsonDump;
            SendDAPEvent(wxEVT_DAP_ATTACH_RESPONSE_EVENT, ptr, json);
        }
        else if (as_response->command  == "")   //codelldb bug (ph 25/10/02)
        {
            // Codelldb is returning the response with an empty "attach" command item
            //{"seq":5,"type":"response","request_seq":2,"success":false,"command":"","message":"Could not attach: no process found with process ID 69","show_user":true}
            wxString jsonDump = json.ToString(false);
            if ((jsonDump.Contains(R"("success":false)"))
                and jsonDump.Contains(R"("message":"Could not attach:)"))
            {
                AttachResponse* ptr = new dap::AttachResponse;
                ptr->jsonDump = jsonDump;
                SendDAPEvent(wxEVT_DAP_ATTACH_RESPONSE_EVENT, ptr, json);
...
// ----------------------------------------------------------------------------
void dap::Client::StepIn(int threadId, bool singleThread, SteppingGranularity granularity)
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
{
    if (not IsSupported(supportsBreakpointLocationsRequest))
    {
        return;
    }
...
void dap::Client::GetDisassemble(uint64_t memAddr, int offset, int instructionOffset,
                                 int instructionCount, bool resolveSymbols)
// ----------------------------------------------------------------------------
{
    DisassembleRequest req = MakeRequest<DisassembleRequest>();

    wxString memStr = wxString::Format("0x%llx", memAddr);
    req.arguments.memoryReference = memStr.ToStdString();
    req.arguments.offset = offset;
    req.arguments.instructionOffset = instructionOffset;
    req.arguments.instructionCount = instructionCount;
    req.arguments.resolveSymbols = true;
    SendRequest(req);
}
// ----------------------------------------------------------------------------
void dap::Client::GetVariableInfo(int variablesReference, wxString filter, int start, int count,
                        ValueDisplayFormat format) //(ph 2024/10/10)
// ----------------------------------------------------------------------------
{
    VariablesRequest req = MakeRequest<VariablesRequest>();
    req.arguments.variablesReference = variablesReference;
    req.arguments.count = count;
    req.arguments.format.hex = (format == ValueDisplayFormat::HEX);
    req.arguments.filter = filter;
    //-m_get_variables_queue.push_back({ variablesReference, context });
    SendRequest(req);
    //-return req.seq;
}
// ----------------------------------------------------------------------------
void dap::Client::Goto(int threadId, int targetId) //(ph 2024/11/08)
// ----------------------------------------------------------------------------
{
    GotoRequest req = MakeRequest<GotoRequest>();

    req.arguments.threadId = threadId;
    req.arguments.targetId = targetId;
    SendRequest(req);
}
// ----------------------------------------------------------------------------
void dap::Client::GotoTarget(wxString source, int line, int column) //(ph 2024/11/12)
// ----------------------------------------------------------------------------
{
    if (not IsSupported(supportsGotoTargetsRequest))
    {
        return;
    }

    GotoTargetsRequest req = MakeRequest<GotoTargetsRequest>();
    wxFileName srcFN(source);
    req.arguments.source.name = srcFN.GetFullPath();
    req.arguments.source.path = srcFN.GetFullPath(); //not needed, should be removed.
    req.arguments.line = line;
    req.arguments.column = column;
    //-req.arguments.threadId = threadId;
    SendRequest(req);
}
...
// ----------------------------------------------------------------------------
void dap::Client::Disconnect(bool terminate, bool restart)     // (ph 25/10/26)
// ----------------------------------------------------------------------------
{
    DisconnectRequest req = MakeRequest<DisconnectRequest>();
    req.terminateDebuggee = terminate;
    req.restart = restart;
    SendRequest(req);
}

// ----------------------------------------------------------------------------
void dap::Client::Terminate(bool restart)
// ----------------------------------------------------------------------------
{
    TerminateRequest req = MakeRequest<TerminateRequest>();
    req.arguments.restart = restart;
    SendRequest(req);
}
