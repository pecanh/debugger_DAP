...
#define CAPABILITIES_BODY_PROCESS(Type, sType)  \
    if(body[sType].IsOK()) {                    \
        Type =Type = body[sType].GetBool(false);      \
    }
...
void Initialize()
{
    ...
    REGISTER_CLASS(TerminateRequest);
    ...
    REGISTER_CLASS(TerminateResponse);
...
Json StackFrame::To() const
{
    ...
    ADD_PROP(id);
    ADD_PROP(line);
    ADD_PROP(instructionPointerReference); //(ph 2024/08/28)
    ADD_OBJ(source);
    return json;
}
...
void StackFrame::From(const Json & json)
{
    ...
    GET_PROP(instructionPointerReference, String); //(ph 2024/08/28)
    READ_OBJ(source);
}
...
void InitializeResponse::From(const Json & json)
{
    ...
    if (json["body"].IsOK())
    {
        ...
        CAPABILITIES_BODY_PROCESS(capabilities.supportsGotoTargetsRequest, "supportsGotoTargetsRequest") //(ph 2024/11/11)
...
Json TerminateRequest::To() const
{
    Json json = Request::To();
    json.AddObject("arguments", arguments.To());
    return json;
...
void TerminateRequest::From(const Json & json)
{
    Request::From(json);
    arguments.From(json["arguments"]);
}

Json TerminateArguments::To() const
{
    Json json = Json::CreateObject();
    json.Add("restart", restart);
    return json;
}

void TerminateArguments::From(const Json & json)
{
    restart = json["restart"].GetBool(false);
}
...
Json DisassembleArguments::To() const
{
    CREATE_JSON();
    ADD_PROP(memoryReference);
    ADD_PROP(offset);
    ADD_PROP(instructionOffset);
    ADD_PROP(instructionCount);
    ADD_PROP(resolveSymbols);
    return json;
}

void DisassembleArguments::From(const Json & json)
{
//    GET_PROP(name, String);
//    GET_PROP(id, Integer);
//    GET_PROP(line, Integer);
...
Json DisassembleRequest::To() const
{
    REQUEST_TO();
    ADD_OBJ(arguments);
    return json;
...
void DisassembleRequest::From(const Json & json)    //(ph 2024/09/02)
{
    Request::From(json);
    arguments.From(json["arguments"]);
}

Json DisassembleResponse::To() const
{

    auto json = Response::To();
    auto arr = json.AddObject("body").AddArray("instructions");

    for (const auto& instruction : disassembleInfo)
    {
        arr.Add(instruction.To());
    }
...
void DisassembleResponse::From(const Json & json) //(ph 2024/09/02)
{
    Response::From(json);
    auto arr = json["body"]["instructions"];
    size_t count = arr.GetCount();
    disassembleInfo.clear();
    disassembleInfo.reserve(count);

    for (size_t i = 0; i < count; ++i)
    {
        DisassembleInfo di;
        di.From(arr[i]);
        disassembleInfo.push_back(di);
    }
}
Json DisassembleInfo::To() const //(ph 2024/09/02)
{
    CREATE_JSON();
    ADD_PROP(address);
    ADD_PROP(column);
    ADD_PROP(name); //(ph 2024/08/28)
    ADD_PROP(path);
    return json;
}

void DisassembleInfo::From(const Json & json) //(ph 2024/09/02)
{
    GET_PROP(address, String);
    GET_PROP(column, Integer);
    GET_PROP(instruction, String);
    GET_PROP(instructionBytes, String);
    GET_PROP(line, Integer);
    Json location = json["location"];
    name = location["name"].GetString();
    path = location["path"].GetString();
}
...
void GotoArguments::From(const Json & json)
{
}
Json GotoRequest::To() const
{
    REQUEST_TO();
    ADD_OBJ(arguments);
    return json;
...
void GotoRequest::From(const Json & json)    //(ph 2024/09/02)
{
    Request::From(json);
    arguments.From(json["arguments"]);
}

Json GotoResponse::To() const
{
...
void GotoResponse::From(const Json & json) //(ph 2024/09/02)
{
    Response::From(json);
}
// ----------------------------------------
...
Json GotoTargetsArguments::To() const
{
    CREATE_JSON();
    ADD_OBJ(source);
    ADD_PROP(line);
...
void GotoTargetsArguments::From(const Json & json)
{
}
Json GotoTargetsRequest::To() const
{
    REQUEST_TO();
    ADD_OBJ(arguments);
    return json;
...
void GotoTargetsRequest::From(const Json & json)    //(ph 2024/09/02)
{
    Request::From(json);
    arguments.From(json["arguments"]);
}

Json GotoTargetsResponse::To() const
{

    auto json = Response::To();
    auto arr = json.AddObject("body").AddArray("gotoTargets");

    for (const auto& targets : gotoTargetsInfo)
    {
        arr.Add(targets.To());
    }
...
void GotoTargetsResponse::From(const Json & json) //(ph 2024/09/02)
{
    Response::From(json);
    auto arr = json["body"]["targets"];
    size_t count = arr.GetCount();
    gotoTargetsInfo.clear();
    gotoTargetsInfo.reserve(count);

    for (size_t i = 0; i < count; ++i)
    {
        GotoTargetsInfo targets;
        targets.From(arr[i]);
        gotoTargetsInfo.push_back(targets);
    }
}
Json GotoTargetsInfo::To() const //(ph 2024/09/02)
{
    CREATE_JSON();
    ADD_PROP(id);
    ADD_PROP(label);
    ...
    ADD_PROP(endLine);
    ADD_PROP(endColumn);
    ADD_PROP(instructionPointerReference);
    return json;
}

void GotoTargetsInfo::From(const Json & json) //(ph 2024/11/11)
{
    GET_PROP(id, Integer);
    GET_PROP(label, String);
    GET_PROP(line, Integer);
    GET_PROP(column, Integer);
    GET_PROP(endLine, Integer);
    GET_PROP(endColumn, Integer);
