...
#include <wx/socket.h>  //(ph 2024/05/08) Find available socket port
#include <wx/utils.h>   //(ph 2024/08/24) FindWindowbyName()
#include <wx/stattext.h> //(ph 2024/10/15)
...
//#include "infowindow.h" included by plugin.h in DAP_Watches.h
#include "macrosmanager.h"
#include "manager.h"
//#include "pipedprocess.h"
#include "projectmanager.h"
#include "editormanager.h" //(ph 2024/09/02)
#include "cbeditor.h"
...
// ----------------------------------------------------------------------------
    : cbDebuggerPlugin("DAP", "debugger_dap"),
      ...
      m_dapPid(0),
      m_pid_attached(0),
      m_steppingIntoInstruction(false)
{
    ...
    //-Debugger_State::AddState(Debugger_State::eDAPState::NotConnected); //(ph 2024/06/06)
    Debugger_State::RemoveState(Debugger_State::eDAPState::Connected); //(ph 2024/06/06)
    ...
    m_dapClient.Bind(wxEVT_DAP_THREADS_RESPONSE,                &Debugger_DAP::OnThreadsResponse,       this);
    ...
    pDAPCallStack = new DBG_DAP_CallStack(this, m_pLogger, &m_dapClient);
...
// destructor
// ----------------------------------------------------------------------------
Debugger_DAP::~Debugger_DAP() //dtor
// ----------------------------------------------------------------------------
{
    ...
    m_dapClient.Unbind(wxEVT_DAP_THREADS_RESPONSE,                &Debugger_DAP::OnThreadsResponse,          this);
    ...
    if (m_dapPid != 0)
    {
        wxKill(m_dapPid, wxSIGKILL, NULL, wxKILL_CHILDREN);
        m_dapPid = 0;
    }
}
...
// ----------------------------------------------------------------------------
{
    m_appShutDown = false; //(ph 2025/01/24)
    ...
    // Do not use cbEVT_PROJECT_OPEN as the project may not be active!!!!
    ...
    Manager::Get()->RegisterEventSink(cbEVT_EDITOR_OPEN,  new cbEventFunctor<Debugger_DAP, CodeBlocksEvent>(this, &Debugger_DAP::OnEditorOpened)); //(ph 2024/11/06)
}
...
// ----------------------------------------------------------------------------
{
    if (appShutDown) m_appShutDown = true;
...
// ----------------------------------------------------------------------------
{
    // Get Filename and line from the current or chosen stack frame.

    //-Done- m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Functionality supported"), dbg_DAP::LogPaneLogger::LineType::Error);

    filename = wxString(); line = wxNOT_FOUND;
    if ((not Debugger_State::IsRunning()) or (not Debugger_State::IsStopped()) ) //(ph 2024/11/06)
        return;
    ...
    if (not pXtn) return;
    filename = pXtn->GetFilename();
    wxString lineStr = pXtn->GetLine();
    if (lineStr.Length()) line = std::atoi(lineStr);
    return;
}
...
// ----------------------------------------------------------------------------
{
    ...
    pDAPBreakpoints->m_start_type = start_type; //(ph 2024/06/07)
    return (not EnsureBuildUpToDate(start_type) );
}
...
// ----------------------------------------------------------------------------
{
    ...
    target = NULL;
    compiler = NULL;
    wxString active_build_target = wxString(); //(ph 2025/01/23)
...
// ----------------------------------------------------------------------------
{
    ...
    // This allows the RuntoCursor/StepInto/ and Run menu/buttons to be enabled.
    Debugger_State::AddState(Debugger_State::eDAPState::Stopped); //(ph 2024/06/05)
}
...
// ----------------------------------------------------------------------------
{
    ...
    if (!target and (m_pid_attached == 0))
    {
    ...
    wxString dap_debugger = GetActiveConfigEx().GetDAP_Executable(true);
    ...
    // Debugger must be a lldb type debugger
    wxFileName fnDebugger(dap_debugger);
    ...
    // If no DAP port is configured, ask the user if they want to find an available one.
    if (dap_port_number.IsEmpty())
    {
        if (cbMessageBox(_("No DAP port number is set in the project configuration.\nShould I try to find an available port?"), _("DAP Debugger"), wxYES_NO | wxICON_QUESTION) == wxID_YES)
        {
            // Attempt to find an available socket port.
            int intResult = dbg_DAP::Find_available_port();
            if (intResult > 0)
            {
                // Found a port, update the local variable and persist it to the configuration.
                dap_port_number = wxString::Format("%d", intResult);
                GetActiveConfigEx().SetDAP_PortNumber(dap_port_number);
            }
            else
            {
                m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Cannot obtain socket port number!"), dbg_DAP::LogPaneLogger::LineType::Error);
                return 6;
            }
        }
        else
        {
            // User declined to find a port, so we cannot start the debugger.
            return 7;
        }
    }
    ...
    if ((m_pid_attached == 0) and (not GetDebuggee(debuggee, working_dir, target)) ) //(ph 2025/01/23)
    {
        m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Cannot find debuggee!"), dbg_DAP::LogPaneLogger::LineType::Error);
        return 6;
    }
    wxFileName fnPaths = debuggee; //(ph 2024/06/09) remove dots
    fnPaths.Normalize(wxPATH_NORM_DOTS | wxPATH_NORM_ABSOLUTE);
    debuggee = fnPaths.GetFullPath();
    fnPaths = working_dir;
    fnPaths.Normalize(wxPATH_NORM_DOTS | wxPATH_NORM_ABSOLUTE);
    working_dir = fnPaths.GetPath();
    fnPaths = dap_debugger;
    fnPaths.Normalize(wxPATH_NORM_DOTS | wxPATH_NORM_ABSOLUTE);
    dap_debugger = fnPaths.GetFullPath();
    wxString msg = wxString::Format("debugee: %s\nworking_dir: %s\ndap_debugger: %s", debuggee, working_dir, dap_debugger);
    msg.Prepend("StartDebugger()/debuggee/workingDir");
    Manager::Get()->GetLogManager()->DebugLog(msg);

    bool bConsole = m_pid_attached ? false : target->GetTargetType() == ttConsoleOnly; //(ph 2025/01/23)
    // ---------------------------------------------------------------------------------------------------------------------
    ...
    if (bConsole && (not m_pTerminalMgr))
    {
...
{
    //(ph 2024/06/09)
    wxString msg; msg << "dap_debugger:" << dap_debugger << "\n";
    msg << "debuggee:" << debuggee << "\n";
    msg << "dap+port_number:" << dap_port_number << "\n";
    msg << "working_dir:" << working_dir << "\n";
    msg.Prepend("Debugger_Dap::LaunchDebugger() ");
    Manager::Get()->GetLogManager()->DebugLog(msg);
    ...
    if (dap_port_number.IsEmpty())
    {
        m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("dap_debugger port number is empty!!!!"), dbg_DAP::LogPaneLogger::LineType::UserDisplay);
        return 2;
    }

    if ( (not debuggee.IsEmpty()) and (m_pid_attached == 0) ) //(ph 2025/01/23)
    {
    ...
    if (not transport->Connect(connection, 5))
    {
        if (m_dapPid != 0)
        {
            wxKill(m_dapPid, wxSIGKILL, NULL, wxKILL_CHILDREN);
        ...
        wxString msg = wxString::Format("Failed to connect to DAP server.\n using %s" , connection);
        msg << "\nAssure PYTHONHOME environment variable set correctly";
        msg << "\nin Settings/Debugger configuration";
        msg << "\nAssure that the DAP debugger has a --port or --connect parameter.";   // (ph 26/01/19)
        wxMessageBox(msg, "DAP Debugger Plugin", wxICON_ERROR | wxOK | wxCENTRE);
        return 1;
    }

    Debugger_State::AddState(Debugger_State::eDAPState::Connected);
    // construct new client with the transport
    m_dapClient.SetTransport(transport);

    // This part is done in mode **sync**
    //#warning the next stmt may be too soon to set Running
    ...
    ProjectBuildTarget * target;
    SelectCompiler(*project, compiler, target, m_pid_attached); //(ph 2025/01/23)
    wxString debugeeArgs = m_pid_attached ? wxString() : target->GetExecutionParameters(); //(ph 2025/01/23)
    ...
    #ifdef __WXMSW__
    // FIXME (ph#): For windows using CodeLLDB, RunInTerminalRequest stalls debugging. //(ph 2024/12/05)
    //  see OnRunInTerminalRequest() to fix this
        if (dap_debugger.Lower().Contains("codelldb.exe"))
            initArgs.supportsRunInTerminalRequest = false; //(ph 2024/12/05)
    #endif
...
// ----------------------------------------------------------------------------
{
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, "Debugger_DAP::RunToCursor", dbg_DAP::LogPaneLogger::LineType::Debug);
...
// ----------------------------------------------------------------------------
{
    // if not using Codelldb.exe, then not supported yet
    wxString dap_debugger = GetActiveConfigEx().GetDAP_Executable(true);
    if (not dap_debugger.Lower().EndsWith("codelldb.exe"))
    {
    ...
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Functionality only supported for CodeLLDB!"), dbg_DAP::LogPaneLogger::LineType::Error);
    // FIXME (ph#): Check to see if Msys64 and llvm support this yet. //(ph 2024/12/22)
    if (IsStopped())
    {
        ...
        //-int threadId = m_dapClient.GetActiveThreadId();
        int column = 1;
        m_dapClient.GotoTarget(filename, line, column);
    }
}
...
// ----------------------------------------------------------------------------
{
    ...
    m_steppingIntoInstruction = true;
    m_dapClient.StepIn(wxNOT_FOUND, true, dap::SteppingGranularity::INSTRUCTION);
}
...
// ----------------------------------------------------------------------------
{
    ...
    // Send the disconnect command to the DAP debugger
    m_dapClient.Disconnect(true); // (ph 26/01/20)

    // Give the debugger a little time to finish up and close the console
    wxMilliSleep(100);
...
// ----------------------------------------------------------------------------
{
    // Togle breakpoint from cb Editor
...
// ----------------------------------------------------------------------------
{
    // external call from cbPlugin interface or debugger menu handler
    //-return pDAPWatches->AddWatch(symbol, update);    // (ph 25/04/09)
    cb::shared_ptr<cbWatch> pWatch = pDAPWatches->AddWatch(symbol, update);
    if (pWatch and update)
        UpdateWatch(pWatch);
    return pWatch;
}

// ----------------------------------------------------------------------------
cb::shared_ptr<cbWatch> Debugger_DAP::AddWatch(dbg_DAP::DAPWatch* pWatch, cb_unused bool update)
// ----------------------------------------------------------------------------
{
    //-return pDAPWatches->AddWatch(pWatch, update); // (ph 25/04/10)
    pDAPWatches->AddWatch(pWatch, update);
    if (pWatch and update)
    {
        cb::shared_ptr<cbWatch> watchPtr(pWatch); // Implicit upcast
        UpdateWatch(watchPtr);
    }
    return (static_cast<shared_ptr<cbWatch>>(pWatch));
}
...
// ----------------------------------------------------------------------------
{
    // Update watch from menu via watchesdlg::OnMenuUPdate()
    pDAPWatches->UpdateWatch(watch, pDAPCallStack);
}
...
// ----------------------------------------------------------------------------
{
    ...
    //        return false;
    //
    if (not GetActiveConfigEx().GetFlag(dbg_DAP::DebuggerConfiguration::EvalExpression))
        return false;
    ...
    // Called from cbPlugin.cpp to determine if Tooltips are allowed
    if ( (not (m_dapPid > 0)) or (not Debugger_State::IsStopped())
         or (not Debugger_State::IsRunning()) or (not Debugger_State::IsConnected()) )
        return false;
...
{
    // **Debugging**
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, wxString::Format(_("token: %s"), token), dbg_DAP::LogPaneLogger::LineType::Debug);

    EvaluateExpressionWithCallback(token, evalRect);
}
// ----------------------------------------------------------------------------
void Debugger_DAP::EvaluateExpressionCallback(bool success, const wxString& result, const wxString& type, int variablesReference, const wxString& text,const wxRect& evalRect)
// ----------------------------------------------------------------------------
{
    // Shows result of EvaluateExpressionWithCallback() in tooltip
    ...
    if (not shown)
    {
        Manager::Get()->GetDebuggerManager()->GetActiveDebugger()->DeleteWatch(pWatch);
    }
}
// ----------------------------------------------------------------------------
void Debugger_DAP::EvaluateExpressionWithCallback(const wxString& token, const wxRect& evalRect)
// ----------------------------------------------------------------------------
{
    ...
    m_dapClient.EvaluateExpression(
        token, stackID, dap::EvaluateContext::HOVER,
        [this, token, evalRect](bool success, const wxString& result, const wxString& type, int variablesReference) {
            EvaluateExpressionCallback(success, result, type, variablesReference, token, evalRect);
        }
    );
}
// ----------------------------------------------------------------------------
...
// =============================================================================
//      ___                _            __  __
//     | __|_ ____ _ _ __ (_)_ _  ___  |  \/  |___ _ __  ___ _ _ _  _
//     | _|\ \ / _` | '  \| | ' \/ -_) | |\/| / -_) '  \/ _ \ '_| || |
//     |___/_\_\__,_|_|_|_|_|_||_\___| |_|  |_\___|_|_|_\___/_|  \_, |
//                                                               |__/
// =============================================================================

// ----------------------------------------------------------------------------
void Debugger_DAP::GetMemoryWithCallback(const wxString& tokenStr, const int count) //(ph 2024/10/10)
// ----------------------------------------------------------------------------
{
    // To print the address of a variable use
    //      "expr --raw -- &variable" or "frame var -L var-name"
    // Read content of a register
    //      $$eax or "reg read eax"; the $$ is just a flag to use "reg read ..."
    //      p/x $<reg-name> //print hex value
    // To print the memory addressed of a register
    //      memory read -s1 -f x $reg
    // To print data (32 hex bytes) at an address:
    //      x32xb <addr> or x/32x &var-name

    // CB/GDB/CDB debugger always sent this command:
    //    {
    ...
    //    }
    //      by default, we do the same here. It also works for lldb.
    ...
    wxString exprsn = wxString::Format("x/%dxb %s", count, tokenStr);
    if (tokenStr.StartsWith("$$"))
        //exprsn = wxString::Format("reg read %s", tokenStr.Mid(2));    //decimal
        exprsn = wxString::Format("p/x %s", tokenStr.Mid(1));           //hex
    // When the token starts with a '?', use it verbatum.
    if (tokenStr.StartsWith("?") and (tokenStr.Length() > 1))
        exprsn = tokenStr.Mid(1);
    // We'll need the current stack id to use as the lldb environment
    if (pXtn)
        stackID = pXtn->GetDAPStackId();

    // Hand the client our info and a function to execute using our environment
    // See the next function below for the results of the callback.

    m_dapClient.EvaluateExpression( exprsn, stackID, dap::EvaluateContext::REPL,
        [this, exprsn](bool success, const wxString& result, const wxString& type, int variablesReference)
        {
            GetMemoryCallback(success, result, type, variablesReference, exprsn);
        }
    );
}
// ----------------------------------------------------------------------------
void Debugger_DAP::GetMemoryCallback(bool success, const wxString& result, const wxString& type, int variablesReference, const wxString& text)
// ----------------------------------------------------------------------------
{   //(ph 2024/10/10)

    // If the ExamineMemoryDlg is still shown on the screen,
    // fill the Examine memory dialog class with the results of the
    //      DAP/lldb get memory REPL results
    ...
    // create a convenience string from the DAP json results
    output << text << " = " << result;
    if (type.Length()) output << " [" << type  << "]"; //-variablesReference: " << variablesReference;
    Manager::Get()->GetLogManager()->DebugLog(output);

    cbExamineMemoryDlg* pDlg = Manager::Get()->GetDebuggerManager()->GetExamineMemoryDialog();
    wxWindow* pMemoryWin = Manager::Get()->GetDebuggerManager()->GetExamineMemoryDialog()->GetWindow();
    if ((not pMemoryWin) or (not pMemoryWin->IsShownOnScreen()) )
        return;
    wxString addrStr = pDlg->GetBaseAddress();
    uint64_t count = pDlg->GetBytes();
    if (addrStr.empty() or (count == 0))
        return;

    pDlg->Begin();  //freezes the window
    pDlg->Clear();  // clears previous displayed data
    int outted = 0; // flag determines if dialog data was filled
    ...
    {
        outted = 0;
        if (lines[i].Lower().StartsWith("error: "))
        {
            pDlg->AddError(lines[i]);
            continue;
        }
        if (ParseExamineMemoryLine(addr, values, lines[i]))
        {
            wxString hexByte;
            for (uint8_t v : values)
            {
                hexByte = wxString::Format(wxT("%02x"), v);
                pDlg->AddHexByte(addr, hexByte);
                outted++;
            }
        }
        else
        ...
        // if nothing was output but there was data, output the additional data
        // usually an error message or additional lldb info
        if ((not outted) and lines[i].Length())
        {
            // remove the unneeded lldb var ref id if present.
            int posn = lines[i].Find("variablesReference: ");
            if ( wxFound(posn))
                lines[i] = lines[i].Mid(0,posn);
            pDlg->AddError(lines[i]);
        }
    }
...
// regex to find/format hex bytes and ascii info (used below)
const wxRegEx reExamineMemoryLine(wxT("[[:blank:]]*(0x[0-9a-f]+)[[:blank:]]<.+>:[[:blank:]]+(.+)"));

// ----------------------------------------------------------------------------
bool Debugger_DAP::ParseExamineMemoryLine(wxString &resultAddr, std::vector<uint8_t> &resultValues,
...
// ----------------------------------------------------------------------------
{
    ...
    return true;
}//end ParseExamineMemoryLine()
// ----------------------------------------------------------------------------
void Debugger_DAP::SendCommand(const wxString& cmd, bool debugLog)
// ----------------------------------------------------------------------------
{
    // Send a user command from the Debugger log tab command box to the debugger.
    //NOTE: The debugLog param is always set to true below and prints to
    //      the Code::Blocks Debug log tab.
    ...
    if(not Debugger_State::IsStopped())
    {
    ...
    // Execute a command from the Debugger Command box
    SendCommandWithCallback(cmd, /*debugLog*/true);
}
// ----------------------------------------------------------------------------
void Debugger_DAP::SendCommandWithCallback(const wxString& cmdStr, bool debugLog) //(ph 2024/10/10)
// ----------------------------------------------------------------------------
{
    // Send a user command to be evaluated and output returned to a callback

    int activeStackFrame = pDAPCallStack->GetActiveStackFrame();
    if (activeStackFrame < 0) return; //Happens when AttachToProcess() has no stack frame yet.
    cbStackFrame::ConstPointer pStackFrame = pDAPCallStack->GetStackFrame(activeStackFrame);
    const dbg_DAP::cbStackFrameXtn* pXtn = static_cast<const dbg_DAP::cbStackFrameXtn*>(pStackFrame.get());
    int stackID = 0;
    if (cmdStr.empty()) return;
    wxString exprsn = cmdStr;
    // We'll need the current stack id to use as the lldb environment
    if (pXtn)
        stackID = pXtn->GetDAPStackId();
    ...
    m_dapClient.EvaluateExpression( exprsn, stackID, dap::EvaluateContext::REPL,
        [this, exprsn, debugLog](bool success, const wxString& result, const wxString& type, int variablesReference)
        {
            SendCommandCallback(success, result, type, variablesReference, exprsn, debugLog);
        }
    );
}//end SendCommandWithCallback
// ----------------------------------------------------------------------------
void Debugger_DAP::SendCommandCallback(bool success, const wxString& result, const wxString& type, int variablesReference, const wxString& text, bool debugLog)
// ----------------------------------------------------------------------------
{   //(ph 2024/10/19)

    // print the output of the user debugger command
    ...
    {
        if (lines[i].Lower().StartsWith("error: "))
            pLogMgr->DebugLogError(lines[i]);
        else
            pLogMgr->DebugLog(lines[i]);
    }
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, output, dbg_DAP::LogPaneLogger::LineType::UserDisplay);

}//end SendCommandCallback
...
// ----------------------------------------------------------------------------
{
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("AttachToProcess"), dbg_DAP::LogPaneLogger::LineType::Error);
    ...
    m_pid_attached = number;

    StartDebugger(m_pProject, StartTypeUnknown);
    std::vector<wxString> arguments;
    arguments.push_back(wxString::Format("%ld", number));
    m_dapClient.Attach(number, arguments);

    return;
}
// ----------------------------------------------------------------------------
void Debugger_DAP::OnAttachToProcessResponse(DAPEvent & event)
// ----------------------------------------------------------------------------
{
    dap::AttachResponse* resp = event.GetDapResponse()->As<dap::AttachResponse>();
    if (resp)
    {
        if (resp->success)
        {
            m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Attach succeeded!"), dbg_DAP::LogPaneLogger::LineType::UserDisplay);
            // You can update your UI or state here
            // enable the debug command window
            Debugger_State::AddState(Debugger_State::eDAPState::Running); //(ph 2024/06/06)
            // This does not enable the debug menus
            Debugger_State::AddState(Debugger_State::eDAPState::Stopped); //(ph 2024/06/05)
            m_dapPid = m_pid_attached;
            SwitchToDebuggingLayout();
        }
        else
        {
            m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, wxString::Format(_("Attach failed: %s"), resp->message), dbg_DAP::LogPaneLogger::LineType::Error);
            wxMessageBox("Failed to attach: " + resp->message, "DAP", wxICON_ERROR | wxOK | wxOK_DEFAULT | wxCENTRE);
            // Reset state or notify user
            m_pid_attached = 0;
        }
    }
}
...
// ----------------------------------------------------------------------------
{
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("DetachFromProcess"), dbg_DAP::LogPaneLogger::LineType::Error);
    //-    AddStringCommand(wxString::Format("-target-detach %ld", m_executor.GetAttachedPID()));

    if (!IsAttachedToProcess() || !IsRunning())        // (ph 25/10/26)
    {
        m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Cannot detach, not attached to a process or debugger not running."),
                    dbg_DAP::LogPaneLogger::LineType::Error);
        return;
    }

    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Detaching from process..."),
         dbg_DAP::LogPaneLogger::LineType::UserDisplay);

    // Send the disconnect request with terminateDebuggee set to false.
    // This tells the debug adapter to detach without killing the process.
    m_dapClient.Disconnect(false);

    // The OnTerminated or OnConnectionError event will handle the cleanup.
    m_pid_attached = 0;
}
...
// ----------------------------------------------------------------------------
{
    ...
    return m_pid_attached != 0; //(ph 2025/01/24) enables debugger menus
    //-return false; //(ph 2025/01/24)
}
...
// ----------------------------------------------------------------------------
//  called from cbplugin.h
{   //(ph 2024/06/13)

    // Update the User interface windows by issuing get requests to wxDAP client (Client.cpp)
    // The DAP response code will update the windows.

    if ( (not IsRunning()) or (not IsStopped()) ) return;

    switch (window)
    {
        case Backtrace:
        {
            m_dapClient.GetFrames(); //(ph 2024/08/06)
            break;
        }
        ...
        case Threads:
        {
            ...
            m_dapClient.GetThreads(); //(ph 2024/06/13)
            break;
        }

        case CPURegisters:
        {
            // ask for Register values only when the CPU Registers window is shown
            cbCPURegistersDlg* pDlg = Manager::Get()->GetDebuggerManager()->GetCPURegistersDialog();
            wxWindow* pWindow = pDlg->GetWindow();
            if ((not pWindow) or (not pWindow->IsShownOnScreen()) )
                break;
            if (pDAPRegisters and pDAPRegisters->GetRegistersCount() )
                pDlg->Clear();
            // Get the DAP stack id from cbStackFrame extension, (this is not the cb stack index)
            int activeStackFrame = pDAPCallStack->GetActiveStackFrame();
            if (activeStackFrame < 0) return; //(ph 2025/01/26)
            ...
            if (pXtn)
            {
                dapStackID = pXtn->GetDAPStackId();
                m_dapClient.GetScopes(dapStackID);
            }
            break;
        }
        ...
        case Disassembly:
        {
            //-wxWindow* pDisasmWin =  wxFindWindowByName(_("Disassembly"));
            wxWindow* pDisasmWin = Manager::Get()->GetDebuggerManager()->GetDisassemblyDialog()->GetWindow();
            if ((not pDisasmWin) or (not pDisasmWin->IsShownOnScreen()) ) break;
            ...
            cbStackFrame::ConstPointer pStackFrame = pDAPCallStack->GetStackFrame(activeStackFrame);
            uint64_t addr = pStackFrame->GetAddress();
            m_dapClient.GetDisassemble(addr, 0, 0, 16,true );
            break;
        }
        ...
        case ExamineMemory:
        {
            cbExamineMemoryDlg* pDlg = Manager::Get()->GetDebuggerManager()->GetExamineMemoryDialog();
            wxWindow* pMemoryWin = Manager::Get()->GetDebuggerManager()->GetExamineMemoryDialog()->GetWindow();
            if ((not pMemoryWin) or (not pMemoryWin->IsShownOnScreen()) ) break;
            wxString addrStr = pDlg->GetBaseAddress();
            uint64_t count = pDlg->GetBytes();
            if (addrStr.empty()or (count == 0))
                break;
            GetMemoryWithCallback(addrStr, count);
            break;
        }
...
// ----------------------------------------------------------------------------
{
    //-Debugger_State::AddState(Debugger_State::eDAPState::NotConnected);
    Debugger_State::RemoveState(Debugger_State::eDAPState::Connected);
    ...
    if (not m_appShutDown) //Switch layout logs msg and causes crash while shutting down.
    ...
    cbThreadsDlg * pDialogThreads = Manager::Get()->GetDebuggerManager()->GetThreadsDialog(); //(ph 2024/06/15)
    if (pDialogThreads)
    {
        pDAPCallStack->ClearThreads(); //Threads are part of callstack
    }

    //(ph 2024/10/05) ExamineMemory
    cbExamineMemoryDlg* pDialogExamineMemory = Manager::Get()->GetDebuggerManager()->GetExamineMemoryDialog();
    if (pDialogExamineMemory)
    {
        pDialogExamineMemory->SetBaseAddress("");
        pDialogExamineMemory->Clear();
        wxWindow* pWindow = pDialogExamineMemory->GetWindow();
        if (pWindow)
        {
            //Modify the example "(e.g. 0x40" text //(ph 2024/10/15)
            wxString text = "(e.g. 0x401060, or &&variable, or $$eax)";
            wxStaticText* pLabel = wxDynamicCast(pWindow->FindWindowByLabel(text, pWindow), wxStaticText);
            if (pLabel)
            {
                // pLabel won't be found again after this change
                text = text.RemoveLast() + " or ?vecOfInt[3])";
                pLabel->SetLabel(text);
            }
...
//================================================================================================"
//       ____                       ____  __           __
//      / __/ ___ _  _  __  ___    / __/ / /_  ___ _  / /_  ___
//     _\ \  / _ `/ | |/ / / -_)  _\ \  / __/ / _ `/ / __/ / -_)
//    /___/  \_,_/  |___/  \__/  /___/  \__/  \_,_/  \__/  \__/
//                                                                                                "
//================================================================================================"
...
// ----------------------------------------------------------------------------
{
    ...
    if (resp)
    {
        if (resp->success)
        {
            m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Good Launch Response"), dbg_DAP::LogPaneLogger::LineType::UserDisplay);
        }
        else
        {
...
// ----------------------------------------------------------------------------
{
    ...
    if (response_data)
    {
        if (response_data->success)
        {
            ...
            if (not m_pid_attached) //(ph 2025/01/23)
            {
                // When debuggerDap is used to run/debug debuggerDap we need the CodeBlocks BasePath,
                // not the debuggerDap project BasePath, else CodeBlocks.exe fails.
                wxString exeBasePath = m_pProject ? m_pProject->GetBasePath() : wxString(); //(ph 2025/01/23)
                wxFileName fnCmdPath = m_DAP_DebuggeeStartCMD[0];
                //cbMessageBox(m_DAP_DebuggeeStartCMD[0], "m_Dap_DebuggerStartCmd/OnInitializeResponse(");
                if (not wxFileExists(exeBasePath + wxFILE_SEP_PATH + fnCmdPath.GetFullName()))
                {
                    //exePath.Normalize(wxPATH_NORM_DOTS | wxPATH_NORM_ABSOLUTE);
                    //m_DAP_DebuggeeStartCMD[0] = exePath.GetFullPath();
                    exeBasePath = fnCmdPath.GetPath();
                }
                wxString msg; //(ph 2024/06/09) // **Debugging**
                for (size_t ii = 0; ii<m_DAP_DebuggeeStartCMD.size(); ++ii)
                {
                    msg << m_DAP_DebuggeeStartCMD[ii] << "\n";
                }
                msg << "\nBasePath: " << UnixFilename(exeBasePath);
                msg.Prepend("OnInitializeResponse::Launch ");
                Manager::Get()->GetLogManager()->DebugLog(msg);
...
{
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Received event"), dbg_DAP::LogPaneLogger::LineType::UserDisplay);
    Debugger_State::AddState(Debugger_State::eDAPState::Stopped); //(ph 2024/06/06)
    // got stopped event
    dap::StoppedEvent* stopped_data = event.GetDapEvent()->As<dap::StoppedEvent>();

    //(ph 2026/01/24)
    if (m_steppingIntoInstruction)
    {
        m_steppingIntoInstruction = false;
        cbDisassemblyDlg * pDialogDisassembly = Manager::Get()->GetDebuggerManager()->GetDisassemblyDialog();
        if (pDialogDisassembly)
        {
            if (!IsWindowReallyShown(pDialogDisassembly->GetWindow()))
            {
                CodeBlocksDockEvent evt(cbEVT_SHOW_DOCK_WINDOW);
                evt.pWindow = pDialogDisassembly->GetWindow();
                Manager::Get()->ProcessEvent(evt);
            }
        }
    }

    //Clear any previous stack position setting that the user may have set.
    // Esp: switched to or selected a non-active stack position.
    pDAPCallStack->DAPDebuggerResetData(dbg_DAP::ResetDataType::ResetData_All); //(ph 2024/05/31)
    ...
    if (stopped_data)
    {
            ...
            )
        {
            ...
            */
            if (stopped_data->description.empty())
                stopped_data->description = stopped_data->text; //(ph 2025/01/27)
        ...
        else
        {
            ...
            else
            {
                ...
                */
                if (stopped_data->reason.empty())
                    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, wxString::Format(_("Stopped reason: %s not provided"), stopped_data->reason), dbg_DAP::LogPaneLogger::LineType::Error);
                else
                    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, wxString::Format(_("Stopped reason: %s not supported"), stopped_data->reason), dbg_DAP::LogPaneLogger::LineType::Error);
            }

            m_dapClient.GetFrames();
            m_dapClient.GetThreads(); //(ph 2024/06/16)
        }
    }
    ...
    if (stack_trace_data)
    {
        ...
        for (const auto & stack : stack_trace_data->stackFrames)
        {
            ...
            m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__,
                                     __LINE__,
                                     wxString::Format(_("Stack: ID 0x%X , Name: %s , File: %s  %d instnPtr:%s"),
                                                      stack.id,
                                                      stack.name,
            ...
            //?wxString instructionPointerReference = stack.instructionPointerReference;

            if ((not sFileName.IsEmpty()) && (not bFileFound))
            {
...
// ----------------------------------------------------------------------------
{
    ...
    if (not IsRunning()) return; //(ph 2024/06/15)
...
// ----------------------------------------------------------------------------
void Debugger_DAP::OnThreadsResponse(DAPEvent & event) //(ph 2024/06/15)
// ----------------------------------------------------------------------------
{
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Received event"), dbg_DAP::LogPaneLogger::LineType::UserDisplay);

    // Refresh the threads array container
    pDAPCallStack->SetCurrentThreadID(m_dapClient.GetActiveThreadId());
    pDAPCallStack->OnThreadsResponse(event);    //(ph 2024/06/16)

}
// ----------------------------------------------------------------------------
void Debugger_DAP::OnDisassembleResponse(DAPEvent & event) //(ph 2024/08/26)
// ----------------------------------------------------------------------------
{
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Received event"), dbg_DAP::LogPaneLogger::LineType::UserDisplay);

    // Refresh the disassemble array container
    //dap::Response* = event.GetDapResponse();
    dap::DisassembleResponse* pDisassemData = event.GetDapResponse()->As<dap::DisassembleResponse>();
    if (pDisassemData->success)
    {
        cbDisassemblyDlg* pDlg = Manager::Get()->GetDebuggerManager()->GetDisassemblyDialog();
        ConfigManager* pCfgMgr = Manager::Get()->GetConfigManager(wxT("debugger_common"));
        bool mixedMode = pCfgMgr->ReadBool(wxT("/common/disassembly/mixed_mode"), false);

        int activeStackFrame = pDAPCallStack->GetActiveStackFrame();
        if (activeStackFrame < 0) return; //(ph 2025/01/26)

        cbStackFrame::ConstPointer pStackFrame = pDAPCallStack->GetStackFrame(activeStackFrame);
        pDlg->Clear(*pStackFrame);

        m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Received disassemble event"), dbg_DAP::LogPaneLogger::LineType::UserDisplay);
        for (const auto& disasm : pDisassemData->disassembleInfo)
        {
            #ifdef DAP_DEBUG_ENABLE
            m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__,
                                     __LINE__,
                                     wxString::Format(_("disassembleInfo: ip %s , colume: %d , instrn: %s  bytes %s \
                                                        name %s, line %d" ),
                                                      disasm.address,
                                                      disasm.column ? disasm.column: -1,
                                                      disasm.instruction,
                                                      disasm.instructionBytes,
                                                      disasm.name,
                                                      disasm.line
                                                     ),
                                     dbg_DAP::LogPaneLogger::LineType::UserDisplay);
            #endif
            if (mixedMode and (disasm.line > 0))
            {
                long int lineno = disasm.line-1;
                wxString path = disasm.path;
                // Get the cb editor containing this path
                cbEditor* pEditor = Manager::Get()->GetEditorManager()->IsBuiltinOpen(path);
                cbStyledTextCtrl* pControl = pEditor ? pEditor->GetControl() : nullptr;
                // Get the source line of path at lineno
                wxString srcString;
                if (pControl)
                {
                    srcString = pControl->GetLine(lineno);
                    if (srcString.EndsWith("\n"))
                        srcString.RemoveLast(1);
                    if (srcString.EndsWith("\r"))
                        srcString.RemoveLast(1);
                    pDlg->AddSourceLine(lineno, srcString);
                }
            }
...
}//end OnDisassembleResponse
// ----------------------------------------------------------------------------
void Debugger_DAP::OnGotoResponse(DAPEvent & event) //(ph 2024/11/08)
// ----------------------------------------------------------------------------
{
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Received event"), dbg_DAP::LogPaneLogger::LineType::UserDisplay);
...
// ----------------------------------------------------------------------------
void Debugger_DAP::OnGotoTargetsResponse(DAPEvent & event) //(ph 2024/11/12)
// ----------------------------------------------------------------------------
{
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Received event"), dbg_DAP::LogPaneLogger::LineType::UserDisplay);

    // Refresh the disassemble array container
    dap::GotoTargetsResponse* pGotoTargetsData = event.GetDapResponse()->As<dap::GotoTargetsResponse>();
    // **Debugging**
    //- wxString protomsg = pGotoTargetsData->ToString();

    if (pGotoTargetsData->success)
    {
        int activeStackFrame = pDAPCallStack->GetActiveStackFrame();
        if (activeStackFrame < 0) return; //(ph 2025/01/26)
        cbStackFrame::ConstPointer pStackFrame = pDAPCallStack->GetStackFrame(activeStackFrame);

        for (const auto& target : pGotoTargetsData->gotoTargetsInfo)
        {
            #ifdef DAP_DEBUG_ENABLE
            m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__,
                                     __LINE__,
                                     wxString::Format(_("gotoTargetsInfo: id %d , label: %s , line: %d"
                                                    // The rest of the info is missing in the response
                                                    // column %d, endLine %d, endColumn %d"
                                                    ),
                                                      target.id,
                                                      target.label,
                                                      target.line
            ...
            if (target.line > 0)
            {
                // Issue the GotoRequest
                int threadId = m_dapClient.GetActiveThreadId();
                if (threadId > wxNOT_FOUND)
                    m_dapClient.Goto(threadId,target.id);
...
// ----------------------------------------------------------------------------
{
    // Breakpoint response when DAP sets a breakpoint
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Received event"), dbg_DAP::LogPaneLogger::LineType::UserDisplay);
}
...
{
    m_pLogger->LogDAPMsgType(__PRETTY_FUNCTION__, __LINE__, _("Received event"), dbg_DAP::LogPaneLogger::LineType::UserDisplay);
    if (IsStopped() and IsRunning())
        ClearActiveMarkFromAllEditors(); // (ph 25/04/05)
}
...
// "==================================================================================================================="
//       ___  ___   ___    ____              __        ____        __
//      / _ \/ _ | / _ \  / __/  _____ ___  / /____   / __/__  ___/ /
//     / // / __ |/ ___/ / _/| |/ / -_) _ \/ __(_-<  / _// _ \/ _  /
//    /____/_/ |_/_/    /___/|___/\__/_//_/\__/___/ /___/_//_/\_,_/
//
// "==================================================================================================================="
...
// "==================================================================================================================="
//       ____   ___ __             ____             _
//      / __/__/ (_) /____  ____  / __/__ _____  __(_)______ ___
//     / _// _  / / __/ _ \/ __/ _\ \/ -_) __/ |/ / / __/ -_|_-<
//    /___/\_,_/_/\__/\___/_/   /___/\__/_/  |___/_/\__/\__/___/
//
// "==================================================================================================================="

// ----------------------------------------------------------------------------
void Debugger_DAP::OnEditorOpened(CodeBlocksEvent& event)
// ----------------------------------------------------------------------------
{
    // when an editor opens, look if we have breakpoints for it
    // and notify it...
    // This occurs when the current debugger active stack editor is
    // closed and opened again.  //(ph 2024/11/06)
